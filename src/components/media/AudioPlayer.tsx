...
export default function AudioPlayer() {
  ...
  return (
    <motion.div
      ...
      className="fixed bottom-0 left-0 right-0 z-50 glass border-t border-dusk/40"
    ...
    >
      ...
      <div className="relative">
        <input
          ...
          style={{
            background: `linear-gradient(to right, #a78bfa ${(currentTime / (duration || 1)) * 100}%, #3d3d5c ${(currentTime / (duration || 1)) * 100}%)`,
          }}
        />
      </div>
      ...
      <div className="flex items-center gap-4 px-4 py-3">
        ...
        <div className="flex items-center gap-3 w-1/4 min-w-0">
          <div className="w-10 h-10 rounded-lg bg-gradient-to-br from-nebula to-aurora flex-shrink-0 overflow-hidden">
        ...
        <div className="flex-1 flex items-center justify-center gap-4">
          {/* Shuffle */}
          <button onClick={toggleShuffle} className={`p-1.5 rounded-lg transition-colors ${shuffle ? 'text-aurora' : 'text-dusk hover:text-moon'}`}>
          ...
          {/* Repeat */}
          <button onClick={cycleRepeat} className={`p-1.5 rounded-lg transition-colors ${repeat !== 'none' ? 'text-aurora' : 'text-dusk hover:text-moon'}`}>
            {repeat === 'one' ? (
              <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <div className="flex items-center gap-3 w-1/4 justify-end">
          ...
          <select
            ...
            className="bg-ink border border-dusk rounded-lg text-xs text-moon px-2 py-1 focus:outline-none focus:border-aurora hidden sm:block"
          >
            {speeds.map((s) => (
